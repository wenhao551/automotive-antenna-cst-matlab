...
%% =========================================================
%  Read element radiation pattern
%  Excel columns:
%   1: Theta [deg]
%   2: Phi   [deg]
%   3: Abs(Dir) [dBi]
%% =========================================================
T = readtable('SQU_Gain_Theta25.xlsx');
...
% Sort values
[phi_deg, idx_sort] = sort(phi_deg);
G_lin = G_lin(idx_sort);

% Remove duplicated values to avoid interp1 errors
[phi_deg, ia] = unique(phi_deg, 'stable');
G_lin = G_lin(ia);

% Element pattern interpolation function
% Input angle: angle a_rad relative to the +y axis
element_gain = @(a_rad) interp1(phi_deg, G_lin, abs(rad2deg(a_rad)), 'linear', 0);
...
for s = 1:length(steer_deg_list)
    ...
    for m = 1:N
        ...
        % Direction angle from element m to each sample point, relative to the +y axis
        a_m = atan2(dx, dy);

        % Look up the element gain in this direction from the table
        Gm = element_gain(a_m);
